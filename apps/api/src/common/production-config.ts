const REQUIRED_CORE_PRODUCTION_KEYS = [
  "DATABASE_URL",
  "JWT_ACCESS_SECRET",
  "JWT_REFRESH_SECRET",
...
const REQUIRED_INTEGRATION_PRODUCTION_KEYS = [
  "CINETPAY_API_KEY",
  "CINETPAY_SITE_ID",
  "CINETPAY_SECRET_KEY",
  "SMS_API_URL",
...
export function assertProductionConfiguration(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const missing = REQUIRED_CORE_PRODUCTION_KEYS.filter((key) => !env[key]?.trim());
  if (missing.length) {
  ...
  if (env.ALLOW_DEGRADED_PRODUCTION === "true") {
    return;
  }

  const missingIntegrations = REQUIRED_INTEGRATION_PRODUCTION_KEYS.filter(
    (key) => !env[key]?.trim(),
  );
  if (missingIntegrations.length) {
    throw new Error(`Configuration production incomplète : ${missingIntegrations.join(", ")}`);
  }
